...
def assign_cards_by_position(predictions):
    ...
    clustering = DBSCAN(eps=500, min_samples=1).fit(card_positions)
...
while True:
    ...
    for p in predictions:
        if p["class"] not in unique_cards:
            ...
            if p["confidence"] > unique_cards[p["class"]]["confidence"] * 1.1:
                unique_cards[p["class"]] = p
    ...
    hand_state = "No Hand Found" if not player_cards else (
